"""Esquemas Pydantic y validadores para entrada de datos."""
...
class Turno(str, Enum):
    """Turnos de trabajo de enfermería."""
...
class PersonaIn(BaseModel):
    ...
    telefono: str = Field(
        pattern=r"^[0-9+\-\s]{7,15}$", description="Número de teléfono"
    )
    direccion: str = Field(
        min_length=3, max_length=200, description="Dirección de residencia"
    )

    @field_validator("nombre")
    @classmethod
    def validar_nombre(cls, v: str) -> str:
        """Nombre con caracteres permitidos y sin espacios extremos."""
    ...
    def validar_fecha_nacimiento(cls, v: str) -> str:
        """Fecha válida y no futura (dd/mm/yyyy)."""
...
class MedicoIn(PersonaIn):
    """Entrada para médicos."""

    especialidad: str = Field(
        min_length=2, max_length=50, description="Especialidad médica del doctor"
    )


class EnfermeraIn(PersonaIn):
    """Entrada para enfermeras."""

    turno: Turno = Field(description="Turno de trabajo de la enfermera")


class CitaIn(BaseModel):
    """Entrada para citas médicas."""
    ...
    hora: str = Field(
        pattern=r"^\d{2}:\d{2}$", description="Hora de la cita en formato HH:MM"
    )
    motivo: str = Field(
        min_length=5, max_length=200, description="Motivo de la consulta"
    )

    @field_validator("fecha")
    @classmethod
    def validar_fecha_cita(cls, v: str) -> str:
        """Fecha futura válida (dd/mm/yyyy)."""
        fecha_obj = datetime.strptime(v, "%d/%m/%Y")
...
class DiagnosticoIn(BaseModel):
    """Entrada para diagnósticos médicos."""
    ...
    diagnostico: str = Field(
        min_length=5, max_length=200, description="Diagnóstico médico establecido"
    )
    ...
    observaciones: Optional[str] = Field(
        default="", max_length=300, description="Observaciones adicionales del médico"
    )
...
class FacturaIn(BaseModel):
    """Entrada para facturación."""

    concepto: str = Field(
        min_length=5, max_length=100, description="Concepto del servicio prestado"
    )
    monto: float = Field(
        gt=0, le=1000000, description="Monto del servicio (máximo 2 decimales)"
    )
    tipo_servicio: TipoServicio = Field(description="Tipo de servicio prestado")
    fecha_servicio: str = Field(
        pattern=r"^\d{2}/\d{2}/\d{4}$",
        description="Fecha del servicio en formato dd/mm/yyyy",
    )
    descripcion: Optional[str] = Field(
        default="", max_length=200, description="Descripción detallada del servicio"
    )
...
def validar_campo_fecha(fecha: str) -> tuple[bool, str]:
    ...
    if anio_num > fecha_actual.year:
        ...
        anio_num == fecha_actual.year
        and mes_num == fecha_actual.month
        and dia_num > fecha_actual.day
    ):
        return False, "La fecha de nacimiento no puede ser futura"
